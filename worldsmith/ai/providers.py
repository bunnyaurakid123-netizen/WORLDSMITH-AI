...
from .system_prompt_v2 import WORLDsmith_AI_SYSTEM_PROMPT
...
def call_openai(api_key: str, model: str, prompt: str, schema: dict[str,Any]|None=None, reasoning: str="high") -> AIResponse:
    ...
    payload={"model":model or "gpt-5.6","instructions":WORLDsmith_AI_SYSTEM_PROMPT,"input":prompt,"reasoning":{"effort":_effort(reasoning)}}
    if schema: payload["text"]={"format":{"type":"json_schema","name":"world_plan","strict":True,"schema":schema}}
    ...
    return AIResponse("openai",_clean(text),data,data.get("usage"),data.get("model"))
...
def call_gemini(api_key: str, model: str, prompt: str, schema: dict[str,Any]|None=None, reasoning: str="high") -> AIResponse:
    ...
    try:
        client=genai.Client(api_key=api_key); model=model or "gemini-3.8-flash"
        cfg={"thinking_level":_effort(reasoning)}
        ...
        return AIResponse("gemini",_clean(text),None,None,model)
...
def call_ollama(base_url: str, model: str, prompt: str, schema: dict[str,Any]|None=None, reasoning: str="high") -> AIResponse:
    ...
    return AIResponse("ollama",_clean(text),data,{"total_duration":data.get("total_duration"),"eval_count":data.get("eval_count")},model or "gemma3")
