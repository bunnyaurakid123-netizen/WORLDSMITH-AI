...
from .safety import is_air, is_natural_replaceable
...
class ProductionWorldBuilder(AdvancedWorldBuilder):
    ...
    def __init__(self, level, dimension: str = "minecraft:overworld", seed: int = 1337):
        ...
        self._existing_chunks: set[tuple[int, int]] | None = set()
    ...
    def _configure_safety(self, plan: dict) -> None:
        ...
        try:
            self._max_blocks = max(10_000, min(int(safety.get("max_blocks", 1_500_000)), 3_000_000))
        except (TypeError, ValueError):
            self._max_blocks = 1_500_000
        try:
            self._existing_chunks = set(self._raw_level.all_chunk_coords(self.dimension))
        except Exception:
            # Unknown chunk inventory must fail closed: every existing-world write is inspected.
            self._existing_chunks = None
    ...
    def _should_allow_existing_write(self, x: int, y: int, z: int, dimension, version) -> bool:
        if not self._preserve_existing:
            return True
        if self._phase == "terrain":
            return self._allow_terrain_regeneration
        try:
            current, _ = self._raw_level.get_version_block(x, y, z, dimension, version)
            current_name = self._block_name(current)
            # Structures/roads may reshape ordinary natural terrain, but not likely player-built blocks.
            return is_air(current_name) or is_natural_replaceable(current_name)
        except Exception:
            # Never write when the existing block cannot be inspected.
            return False
    ...
    def _guarded_write(self, x, y, z, dimension, version, block, *args, **kwargs) -> bool:
        ...
        if self._preserve_existing:
            # Newly generated chunks are safe to populate. Existing chunks require block inspection.
            existing = self._existing_chunks
            chunk_exists = existing is None or chunk_pos in existing
            if chunk_exists and not self._should_allow_existing_write(x, y, z, dimension, version):
                self._skipped_count += 1
                return False
    ...
    def _tree(self, x: int, y: int, z: int, kind: str = "oak") -> int:
        ...
        for dy in range(height):
            if self.put(x, y + dy, z, wood): changed += 1
    ...
    def generate_terrain(self, cx: int, cz: int, radius: int, base_y: int, mountain_height: int, roughness: float, water: bool = True, vegetation: bool = True) -> tuple[int, int]:
        ...
        for x in range(cx - radius, cx + radius + 1):
            for z in range(cz - radius, cz + radius + 1):
                ...
                top = min(sample.height, sea_level - 1) if water and sample.river else sample.height
                for y in range(base_y, top + 1):
                    if self.put(x, y, z, self._column_blocks(sample, y, top)): changed += 1
                if water and top < sea_level:
                    for y in range(top + 1, sea_level + 1):
                        if self.put(x, y, z, "minecraft:water"): changed += 1
                if vegetation and top < base_y + mountain_height * 0.66 and not sample.river:
                    ...
                    if sample.biome == "wet_forest" and density > 0.68: tree_candidates.append((x, top + 1, z, "oak"))
                    elif sample.biome == "temperate" and density > 0.83: tree_candidates.append((x, top + 1, z, "oak"))
                    elif sample.biome == "highland" and density > 0.90: tree_candidates.append((x, top + 1, z, "spruce"))
                columns += 1
        for x, y, z, kind in tree_candidates[::2][:900]: changed += self._tree(x, y, z, kind)
    ...
    def build(self, plan: dict):
        ...
        result = BuildResult()
        self._phase = "terrain"
        if terrain.get("enabled", True): self.generate_terrain(cx, cz, radius, cy, mountain_height, roughness, bool(terrain.get("water", True)), bool(terrain.get("vegetation", True)))
        self._phase = "caves"
        ...
        self._phase = "roads"
        for road in plan.get("roads", [])[:48]: self.road(int(road.get("x1", cx)), int(road.get("z1", cz)), int(road.get("x2", cx)), int(road.get("z2", cz)), int(road.get("y", cy + 3)), int(road.get("width", 3)))
        self._phase = "bridges"
        from .bridges import BridgeBuilder
        bridge_builder = BridgeBuilder(self.level, self.dimension)
        for bridge in plan.get("bridges", [])[:16]: bridge_builder.build(bridge)
        ...
        for build in plan.get("builds", [])[:24]:
            ...
            result.roads_changed += built.roads_changed; result.systems_changed += built.systems_changed; result.structures_changed += built.structures_changed; result.interiors_changed += built.interiors_changed
        self._phase = "finishing"
        from .decoration import ArchitectureFinisher
        finisher = ArchitectureFinisher(self.level, self.dimension)
        for build in plan.get("builds", [])[:24]:
            report = finisher.finish(build); result.interiors_changed += report.interior_blocks
    ...
    def redstone_gate(self, x: int, y: int, z: int) -> int:
        ...
        if not report.valid: raise RuntimeError(report.message)
